...
class Config:
    def __init__(self) -> None:
        ...
        self.company_name = str(os.getenv("CompanyName", "")).strip()
        self.company_address = str(os.getenv("AddresCompany", os.getenv("AddressCompany", ""))).strip()
        self.footer_message = str(os.getenv("Message", "")).strip()
...
class TicketPrinter:
    ...
    def _build_document_bytes(self, order: Dict[str, Any], center: str, details: List[Dict[str, Any]]) -> bytes:
        ...
        item_lines.append("DESCRIPCION".center(TICKET_WIDTH))
        item_lines.append("-" * TICKET_WIDTH)
        for detail in details:
            product = (detail.get("Product") or "").strip()
            item_lines.extend(self._format_print_detail_rows(product))
        ...
        if self.config.footer_message:
            for chunk in textwrap.wrap(self.config.footer_message, width=TICKET_WIDTH, break_long_words=False):
                footer_lines.append(chunk.center(TICKET_WIDTH))
    ...
    def _large_centered_line(self, text: str) -> bytes:
        align_center = b"\x1b\x61\x01"
        align_left = b"\x1b\x61\x00"
        size_on = b"\x1d\x21\x22"
        size_off = b"\x1d\x21\x00"
        bold_on = b"\x1b\x45\x01"
        bold_off = b"\x1b\x45\x00"
        return align_center + size_on + bold_on + self._encode_text(text.strip()) + bold_off + size_off + align_left

    def _format_print_detail_rows(self, description: str) -> List[Any]:
        if self.config.detail_use_highlight_style:
            return [self._highlight_line(row) for row in self._format_sized_detail_rows(description)]

        if not self.config.detail_size_command_hex:
            return self._format_detail_rows(description)
        ...
        detail_rows = self._format_sized_detail_rows(description)
    ...
    def _format_detail_rows(self, description: str) -> List[str]:
        wrapped_description = textwrap.wrap(description, width=TICKET_WIDTH) or [""]
        return [str(chunk or "").center(TICKET_WIDTH) for chunk in wrapped_description]

    def _format_sized_detail_rows(self, description: str) -> List[str]:
        detail_width = max(1, self.config.detail_ticket_width)
        ...
        return [str(chunk or "").strip()[:detail_width].center(detail_width) for chunk in wrapped_description]
